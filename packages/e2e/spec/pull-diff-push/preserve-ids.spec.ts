...
describe('Pull and check if ids are preserved for some collections', () => {
  ...
  it('should preserve some uuid from Directus', async () => {
    ...
    expect(original.dashboard.id.toString()).not.toBe(getSyncId(collections.dashboards));
    expect(original.operation.id.toString()).not.toBe(getSyncId(collections.operations));
    expect(original.panel.id.toString()).not.toBe(getSyncId(collections.panels));
    expect(original.role.id.toString()).not.toBe(getSyncId(collections.roles));
    expect(original.permission.id.toString()).not.toBe(getSyncId(collections.permissions));
    expect(original.preset.id.toString()).not.toBe(getSyncId(collections.presets));
    expect(original.translation.id.toString()).not.toBe(
      getSyncId(collections.translations),
    );
    expect(original.webhook.id.toString()).not.toBe(getSyncId(collections.webhooks));
  });
});
