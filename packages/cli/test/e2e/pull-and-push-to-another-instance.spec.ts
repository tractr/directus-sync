...
describe('Pull and push to another instance', () => {
  ...
  const otherInstance = new DirectusInstance();
  const directus = instance.getDirectusClient();
  const otherDirectus = otherInstance.getDirectusClient();
  let sync: DirectusSync;
  let otherSync: DirectusSync;
  ...
  beforeAll(async () => {
    ...
    await otherDirectus.loginAsAdmin();
    sync = new DirectusSync({
      token: await directus.requireToken(),
      url: directus.getUrl(),
      dumpPath: dumpPath,
    });
    otherSync = new DirectusSync({
      token: await otherDirectus.requireToken(),
      url: otherDirectus.getUrl(),
      dumpPath: dumpPath,
    });
  ...
  it('should detect diff', async () => {
    const output = await otherSync.diff();
  ...
  it('should recreate entries in the other Directus, same as original, and remove no dangling ids', async () => {
    // Push the data back to Directus and analyze the output
    const client = otherDirectus.get();
    const output = await otherSync.push();
    const collections = getSystemCollectionsNames();
    expect(output).toContainEqual(info('[snapshot] No changes to apply'));
    ...
    for (const collection of collections) {
      expect((await otherDirectus.getSyncIdMaps(collection)).length).toBe(1);
    }
