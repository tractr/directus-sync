...
export function getDiffMessage(snapshotDiff: SnapshotDiff) {
  ...
  if (snapshotDiff.relations.length > 0) {
    ...
    for (const {
      ...
      related_collection,
      diff,
    } of snapshotDiff.relations) {
      const relatedCollection = formatRelatedCollection(related_collection);

      if (diff[0]?.kind === DiffKind.EDIT) {
        lines.push(
          `  - ${chalk.magenta('Update')} ${collection}.${field}${relatedCollection}`,
        );
        ...
        lines.push(
          `  - ${chalk.red('Delete')} ${collection}.${field}${relatedCollection}`,
        );
      } else if (diff[0]?.kind === DiffKind.NEW) {
        lines.push(
          `  - ${chalk.green('Create')} ${collection}.${field}${relatedCollection}`,
        );
      } else if (diff[0]?.kind === DiffKind.ARRAY) {
        lines.push(
          `  - ${chalk.magenta('Update')} ${collection}.${field}${relatedCollection}`,
